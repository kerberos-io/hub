...
import labels from "./labels_yolov8.json";
...
const preprocess = (source, modelWidth, modelHeight) => {
  ...
  const input = tf.tidy(() => {
    // Try to convert image to tensor
    try {
      const img = tf.browser.fromPixels(source);
      if(img == null || img.shape == null) {
        return null;
      }
...
export const detect = async (source, model, canvasRef, callback = () => {}) => {
  ...
  try {
    ...
    const [modelWidth, modelHeight] = model.inputShape.slice(1, 3); // get model width and heighthape);
